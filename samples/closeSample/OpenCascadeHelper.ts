...
import { Vector3, VertexAttributeName } from "@orillusion/core";
...
export class OpenCascadeHelper {
  ...
  static LengthOfCurve(geomAdaptor, UMin, UMax, segments = 5) {
    let point1 = new Vector3(), point2 = new Vector3(), arcLength = 0, gpPnt = new oc.gp_Pnt();
    for (let s = UMin; s <= UMax; s += (UMax - UMin) / segments) {
      ...
      if (s == UMin) {
        ...
        arcLength += Vector3.distance(point1, point2);
      }
      point2.copy(point1);
    }
    return arcLength;
  ...
  static visualize(id: string, shapeList: any[]) {
    let openCascade = oc;
    let i = 0;
    let geometries = []

    let maxDeviation = 0.1;
    let fullShapeEdgeHashes = {};
    let fullShapeFaceHashes = {};
    let fullShapeEdgeHashes2 = {};
    let facelist: GeoInfo[] = [];
    let edgeList = [];
    let triangulations = [];
    let uv_boxes = [];
    let curFace = 0;
    ...
    try {
      OcLib.ForEachFace(shape, (faceIndex, myFace) => {
        ...
        OcLib.ForEachEdge(myFace, (index, myEdge) => {
      ...
      OcLib.ForEachEdge(shape, (index, myEdge) => {
        let edgeHash = myEdge.HashCode(100000000);
        if (!fullShapeEdgeHashes2.hasOwnProperty(edgeHash)) {
          let this_edge = {
            vertex_coord: [],
    ...
    for (let i = 0; i < facelist.length; i++) {
      const element = facelist[i];
      let geo = this.getGeo(id, i, element);
      geometries.push(geo);
    }
  ...
  public static getGeo(id: string, i, geoInfo: GeoInfo) {
    let indices = geoInfo.tri_indexes;
    let vertices = geoInfo.vertex_coord;
    let normals = geoInfo.normal_coord;
    ...
    geo.indicesBuffer.upload(new Uint16Array(indices), true);
